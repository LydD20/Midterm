...
def test_load_from_empty_existing_file(tmp_path):
    '''Tests loading history from existing empty file'''
    empty_file = tmp_path / "empty.csv"
    empty_file.write_text("") #this creates empty file
...
def test_remove_entry(manage_history):
    ...
    manage_history.delete(0) # removes first entry
    loaded_history= manage_history.load()
...
def test_remove_nonexistent_entry(manage_history):
    ...
    manage_history.delete(5) # attempts to remove entry with non-existent index
    loaded_history = manage_history.load()

    assert len(loaded_history) == 1 

def test_clear_all_entries(manage_history):
    '''Tests clearing everything from history'''
    entry= {
        'index': 1,
        'name': fake.name(),
        'operation': 'add',
    ...
    assert loaded_history.empty # should be empty