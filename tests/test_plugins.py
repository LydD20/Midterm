...
@pytest.mark.parametrize("operation_name, expected_result", [
    ...
    ("divide", lambda a, b: a/b if b !=0 else pytest.raises(ZeroDivisionError)),
...
def save_rand_history(manage_command_fix, rand_numbers):
    '''Helper functinon that saves random history for testing'''
    for indx, (a, b) in enumerate(rand_numbers):
        ...
        manage_command_fix.save_rand_history(data)
...
def test_clear_histoty(manage_command_fix, rand_numbers):
